...
class TestUserDecorator(unittest.TestCase):
    def setUp(self):
        ...
        # followers: List of follower objects (used for accessing individual
        # followers)
        self.mock_user.followers = [MagicMock() for _ in range(100)]
        # follower_count: Integer count of followers (used for quick size
        # checks)
    ...
    def test_sponsored_user_decorator(self):
        ...
        self.assertEqual(
            sponsored_user.get_bio(), f"Sponsored by {company_name}"
        )
    ...
    def test_nested_decorators(self):
        ...
        self.assertEqual(
            verified_sponsored_user.get_handle(), "test_user ✔️ [Sponsored]"
        )

        # Check if bio shows sponsor info (last decorator takes precedence)
        self.assertEqual(
            verified_sponsored_user.get_bio(), f"Sponsored by {company_name}"
        )
    ...
    def test_verified_user_property_forwarding(self):
        ...
        self.assertTrue(
            all(isinstance(f, MagicMock) for f in verified_user.followers)
        )
    ...
    def test_verified_user_signal_forwarding(self):
        ...
        self.assertIs(
            verified_user.follower_added, self.mock_user.follower_added
        )
        self.assertIs(
            verified_user.follower_removed, self.mock_user.follower_removed
        )
        self.assertIs(verified_user.post_created, self.mock_user.post_created)
        self.assertIs(
            verified_user.reputation_changed,
            self.mock_user.reputation_changed,
        )
    ...
    def test_verified_user_method_forwarding(self):
        ...
        self.mock_user.add_follower.assert_called_once_with(
            test_follower, test_post
        )
        ...
        self.mock_user.create_post.assert_called_once_with(
            test_content, test_image
        )
    ...
    def test_verified_user_reputation_recovery(self):
        ...
        self.mock_user.update_reputation_recovery.assert_called_once_with(
            test_time
        )
