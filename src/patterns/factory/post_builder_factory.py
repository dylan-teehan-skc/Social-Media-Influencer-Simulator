...
class BasePostBuilder:
    ...
    def build(self):
        ...
        if hasattr(self, "_sentiment"):
            ...
            self.logger.info(
                f"Set post sentiment to {self._sentiment.name} during build"
            )
...
class TextPostBuilder(BasePostBuilder):
    ...
    def build(self):
        ...
        post = super().build()
        self.logger.info(
            f"TextPostBuilder: Built post for author {
                post.author.handle if post.author else 'None'} with content length {
                len(
                    post.content)}"
        )
        return post
...
class ImagePostBuilder(BasePostBuilder):
    ...
    def build(self):
        ...
        post = super().build()
        self.logger.info(
            f"ImagePostBuilder: Built post for author {
                post.author.handle if post.author else 'None'} with image {
                self._image_path}"
        )
        return post
