...
class FollowerController(QObject):
    ...
    def _create_initial_followers(self):
        ...
        self._logger.info(
            f"Created initial followers: {[f.handle for f in initial_followers]}"
        )
    ...
    def _generate_and_process_followers(self, post, follow_chance):
        ...
        potential_count = self.MIN_FOLLOWERS_PER_POST + int(
            (adjusted_follow_chance / 100)
        ...
        self._logger.debug(
            f"Generating {potential_count} potential followers for post with follow chance {follow_chance}%"
        )
        ...
        if gained_followers > 0:
            self._logger.info(
                f"Gained {gained_followers} new followers from post"
            )
        else:
    ...
    def _create_potential_follower(self, index):
        ...
        if (
            ...
            if random.randint(1, 100) <= 70:
                return Follower.create_with_random_handle(
                    self._current_post_sentiment
                )
