...
class PostCreationInterceptor(ContentInterceptor):
    ...
    def intercept(self, post: Post) -> None:
        ...
        """
        self.logger.info(f"PostCreationInterceptor: Processing post creation: {post.content[:30]}...")
        ...
        if not post.content or post.content.strip() == "":
            ...
            self.logger.warning("PostCreationInterceptor: Empty post content detected")
            return
        ...
        if len(post.content) < 5:
            ...
            # Create warning message
            warning_msg = f"Post too short: Your post is only {len(post.content)} characters long (minimum 5 characters)"
            
            # Add warning to dispatcher if available
            if hasattr(post, '_dispatcher') and post._dispatcher:
                post._dispatcher.add_warning(warning_msg)
                
            self.logger.warning(f"PostCreationInterceptor: Post content too short: {len(post.content)} chars")
            return
        ...
        if len(post.content) > 500:
            ...
            # Create warning message
            warning_msg = f"Post too long: Your post is {len(post.content)} characters long (maximum 500 characters)"
            
            # Add warning to dispatcher if available
            if hasattr(post, '_dispatcher') and post._dispatcher:
                post._dispatcher.add_warning(warning_msg)
                
            self.logger.warning(f"PostCreationInterceptor: Post content too long: {len(post.content)} chars")
            return
