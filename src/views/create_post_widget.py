...
class CreatePostWidget(QWidget):
    ...
    def create_post(self):
        ...
        if not content:
            QMessageBox.warning(self, "Empty Post", "Please enter some content for your post.")
            return
        ...
        if self.user_controller:
            ...
            was_sponsored = isinstance(original_user, SponsoredUser) or hasattr(original_user, 'company_name')
            
            # Process the post and check for sponsorship changes
            updated_user, message = company_service.on_post_created(self.user_controller.user, post)
            
            # Check if sponsorship was lost
            sponsorship_lost = was_sponsored and not (isinstance(updated_user, SponsoredUser) or hasattr(updated_user, 'company_name'))
            
            # Always update the user controller's reference with the possibly updated user
            ...
            if main_controller:
                ...
                # If the sponsorship was lost, show termination message before updating UI
                if sponsorship_lost or (message and "terminated" in message):
                    QMessageBox.warning(
                        self, 
                        "Sponsorship Terminated", 
                        message or "Your sponsorship has been terminated due to content that conflicts with your sponsor's values."
                    )
                    
                    # Force a complete refresh of the UI after sponsorship termination
                    if hasattr(main_controller, 'main_window'):
                        ...
                        if hasattr(main_controller.main_window, 'news_widget'):
                            main_controller.main_window.news_widget.user = updated_user
                ...
                # For non-termination cases, just update normally
                    ...
                    # Always update the news tab to reflect any sponsorship changes
                    if hasattr(main_controller.main_window, 'news_widget'):
                        main_controller.main_window.news_widget.update_user(updated_user)
                        main_controller.main_window.news_widget.update()

                    # Show warning message if there is one but sponsorship wasn't terminated
                    if message and "Warning" in message:
                        QMessageBox.warning(self, "Sponsorship Warning", message)
            ...
            QMessageBox.warning(self, "Error", "Could not create post - user controller not set.")
