...
from src.views.user_profile_widget import (
...
class PostWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Add spacing between avatar and author info
        ...
        if (
            self.post
            and self.post.author
            and hasattr(self.post.author, "profile_picture_path")
            and self.post.author.profile_picture_path
        ):
        ...
        if not profile_pixmap.isNull():
            circular_pixmap = create_circular_pixmap(
                profile_pixmap, size=40, border_size=2
            )
        ...
        # Reduce spacing between handle and timestamp
        author_info_layout.setSpacing(2)

        self.author_label = QLabel(
            f"@{self.post.author.handle}" if self.post.author else "Anonymous"
        )
        self.author_label.setStyleSheet("font-weight: bold;")
        author_info_layout.addWidget(self.author_label)

        self.timestamp_label = QLabel(
            self.post.timestamp.strftime("%Y-%m-%d %H:%M")
        )
        ...
        stats_layout.addWidget(
            self.sentiment_label, alignment=Qt.AlignmentFlag.AlignRight
        )
        ...
        if hasattr(self.post, "sentiment_changed"):
            self.post.sentiment_changed.connect(
                lambda _: self.update_sentiment_label()
            )
    ...
    def comment_on_post(self):
        ...
        if self.post and self.post_controller:
            ...
            self.post_controller.comment_on_post(
                self.post, "Great post!", sentiment, "user"
            )
    ...
    def create_comment_widget(self, comment):
        ...
        header_layout.addWidget(
            timestamp_label, alignment=Qt.AlignmentFlag.AlignRight
        )
