...
def get_base_profile_picture_path():
    ...
    views_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(
        views_dir, "user_data", "profile_pictures", "base_profile_picture.png"
    )
...
def create_circular_pixmap(
    pixmap, size=100, border_size=3, border_color="#888888"
):
...
class ProfileEditDialog(QDialog):
    ...
    def __init__(self, user, parent=None):
        ...
        self.temp_profile_picture_path = (
            None  # Store the selected picture path
        )
        self.init_ui()
    ...
    def init_ui(self):
        ...
        if (
            hasattr(self.user, "profile_picture_path")
            and self.user.profile_picture_path
        ):
        ...
        main_layout.addWidget(
            button_box, alignment=Qt.AlignmentFlag.AlignCenter
        )
    ...
    def save_profile_picture(self):
        ...
        # If it's the base profile picture, just return its path (don't copy
        # it)
        ...
        if (
            hasattr(self.user, "profile_picture_path")
            and self.temp_profile_picture_path
            == self.user.profile_picture_path
        ):
            return self.temp_profile_picture_path

        # Generate a unique filename
        filename = (
            f"profile_{self.user.handle}_{int(datetime.now().timestamp())}.png"
        )
...
class UserProfileWidget(QWidget):
    ...
    def update_user(self, user):
        ...
        self.handle_label.setText(
            f"@{self.user.get_handle() if hasattr(self.user, 'get_handle') else self.user.handle}"
        )

        # Update the displayed bio
        self.bio_text.setText(
            self.user.get_bio()
            if hasattr(self.user, "get_bio")
            else self.user.bio
        )
    ...
    def init_ui(self):
        ...
        avatar_layout.addWidget(
            QLabel("Profile Picture"), alignment=Qt.AlignmentFlag.AlignCenter
        )
        ...
        self.bio_text.setStyleSheet(
            "padding: 10px; background-color: rgba(0, 0, 0, 0.05); border-radius: 5px;"
        )
        ...
        followers_layout.addWidget(
            followers_icon_label, alignment=Qt.AlignmentFlag.AlignCenter
        )
        ...
        posts_layout.addWidget(
            posts_icon_label, alignment=Qt.AlignmentFlag.AlignCenter
        )
    ...
    def update_follower_count(self):
        ...
        if hasattr(self, "follower_count") and self.user:
            ...
            if (
                hasattr(self.user, "get_handle")
                and "✔️" in self.user.get_handle()
            ):
                self.follower_count.setText(
                    f"{count} Followers\n(Verified Account ✔️)"
                )
            else:
    ...
    def update_post_count(self):
        ...
        if hasattr(self, "posts_count_label") and self.user:
            ...
            post_count = (
                len(self.user.posts) if hasattr(self.user, "posts") else 0
            )
