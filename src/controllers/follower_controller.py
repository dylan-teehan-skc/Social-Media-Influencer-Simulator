...
from src.models.post import Sentiment, Comment, Post
from src.services.logger_service import LoggerService
from src.patterns.command.post_commands import CommentCommand, LikeCommand, ShareCommand
import random
...
class FollowerController:
    ...
    def create_random_follower(self, sentiment=None):
        ...
        self.logger.info(f"Created new follower: {follower.handle} with {sentiment.name} sentiment")
        return follower
    ...
    def create_followers_batch(self, count, sentiment_distribution=None):
        ...
        if abs(total - 1.0) > 0.01:  # Allow small rounding errors
            self.logger.warning(f"Sentiment distribution does not sum to 1.0: {total}")
            # Normalize the distribution
            sentiment_distribution = {k: v/total for k, v in sentiment_distribution.items()}
    ...
    def should_follow(self, follower, post, follow_chance):
        ...
        follow_chance = min(100, follow_chance * 2)  # Double the chance (max 100%)
        
        # Log the follow chance
        self.logger.debug(f"Follower {follower.handle} considering following with chance {follow_chance}%")
        
        # For neutral posts, any follower might follow
        if post.sentiment == Sentiment.NEUTRAL:
            result = random.randint(1, 100) <= follow_chance
            if result:
                self.logger.info(f"Follower {follower.handle} will follow due to neutral post")
            return result
        ...
        if is_aligned:
            ...
            result = random.randint(1, 100) <= (follow_chance * 0.3)  # 70% reduction
        
        if result:
            self.logger.info(f"Follower {follower.handle} will follow due to {'aligned' if is_aligned else 'non-aligned'} post")
    ...
    def process_follower_interaction(self, follower, post):
        ...
        if random.randint(1, 100) <= comment_chance:
            comment_text = self.get_comment_for_alignment(alignment, post.sentiment)
            comment = Comment(comment_text, follower.sentiment, follower.handle)
            comment_command = CommentCommand(post, comment)
            comment_command.execute()
            ...
            self.logger.info(f"Follower {follower.handle} commented on post: {comment_text[:30]}...")
    ...
    def calculate_alignment(self, follower, post):
        ...
        if post.sentiment == Sentiment.LEFT:
            alignment = 100 - follower.political_lean  # Higher political_lean = less aligned with LEFT
        elif post.sentiment == Sentiment.RIGHT:
            alignment = follower.political_lean  # Higher political_lean = more aligned with RIGHT
        else:
        ...
        return max(0, min(100, alignment))  # Ensure alignment is between 0 and 100
    ...
    def adjust_lean_from_sentiment(self, follower, sentiment):
        ...
        follower.political_lean = max(0, min(100, follower.political_lean + adjustment))
        
        if old_lean != follower.political_lean:
            self.logger.info(f"Follower {follower.handle} political lean adjusted from {old_lean} to {follower.political_lean} due to {sentiment.name} content")
    ...
    def should_unfollow(self, follower, post):
        ...
        self.logger.debug(f"Follower {follower.handle} (political lean: {follower.political_lean}) has {alignment}% alignment with {post.sentiment.name} post")
        ...
        if should_unfollow:
            self.logger.info(f"Follower {follower.handle} (political lean: {follower.political_lean}) decided to unfollow due to low alignment ({alignment}%) with {post.sentiment.name} post")
    ...
    def update_follower(self, follower, subject, post=None):
        ...
        if post:
            ...
            if self.should_unfollow(follower, post):
                self.logger.info(f"Follower {follower.handle} is unfollowing due to disagreement with post")
    ...
    def calculate_follow_chance(self, user, post_sentiment):
        ...
        if post_sentiment == Sentiment.NEUTRAL:
            ...
            base_chance = user.HOT_TAKE_CHANCE * 1.5  # 50% boost for political posts
